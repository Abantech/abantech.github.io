...
define( 'Command Issuance and Control/CommandIssuanceAndControl', ['postal', 'Asset Management and Inventory/AssetManager', 'Utils/GuidGenerator'], function ( bus, ami, guidGenerator )
{
        ...
        var subscription =
            bus.subscribe( {
                ...
                callback: function ( data, envelope )
                {
                    ...
                    {
                        func = getBridge( actionToFunctionMapping.Bridge )[mapping.Action];
                    }
    ...
    return {
        Initialize: function ()
        {
            ...
            actionToFunctionMapping = Efficio.Configuration.ActionToFunctionMapping || {};

            actionToFunctionMapping.ActionMappings.forEach( AddActionMapping );
...
;
define( 'Input/LeapMotion/FirebaseLeap', ['postal', 'firebase', 'Input/DeviceManager', 'Input/LeapMotion/HelperFunctions'], function ( bus, Firebase, deviceManager, helper )
{
    ...
    function ExtendHands( hands )
    {
        ...
        for ( var property in extensions )
        {
            ...
            {
                hands[i][property] = extensions[property];
            }
        }
    ...
    function ExtendFingers( fingers )
    {
        var extensions = {
            GetFingerLabel: GetFingerLabel,
            DistanceToFinger: DistanceToFinger,
        ...
        for ( var property in extensions )
        {
            ...
            {
                fingers[i][property] = extensions[property];
            }
        }
    ...
    return {
        Initialize: function ( LeapConfiguration )
        {
            ...
            controller = new Firebase( 'https://burning-heat-9165.firebaseIO.com/' );
        ...
        Start: function ()
        {
            controller.on( 'value', function ( data )
            {
                var frame = JSON.parse( data.A.B )

                ExtendHands( frame.hands );
                ExtendFingers( frame.pointables );
...
function ( hirp, ami, constraintsEngine, comm, eventManager, internalScene, sysNotificationListener, deviceManager, metrics, bus )
{
    ...
    return {
        Initialize: function ( EfficioConfiguration )
        {
            ...
            {
                if ( EfficioConfiguration.Devices.LeapMotion.UseFirebase )
                {
                    require( ['Input/LeapMotion/FirebaseLeap'], function ( leap )
                    {
